...
> **Updated per LLD feedback review [date: 2026-08-31T11:40:35Z]**
> Architecture: one mv-data-format plugin (no shim), generic DerivedDataFormat SPI delegation,
> MVCompiledDefinition (programmatic, not dynamic settings), TargetRoutingSnapshot via ClusterStateListener,
> ordered projection with stable aliases from MVCompiledDefinition,
> one node-level NodeDerivedPullService, one poller per primary shard.