...
public class DocumentLookupServiceTests extends OpenSearchTestCase {
    ...
    public void testGetById_successFiltersReservedAndUnderscoreFields() throws Exception {
        ...
        when(executor.executeSingleRow(0L, fs)).thenReturn(
            row("_id", "doc1", "_seq_no", 42L, "_primary_term", 2L, "_version", 5L, "_custom", "internal", "name", "alice", "age", 30)
        );
        ...
        assertFalse("reserved fields excluded: " + source, source.contains("_seq_no"));
        assertFalse("reserved fields excluded: " + source, source.contains("_version"));
        assertFalse("underscore-prefixed fields excluded: " + source, source.contains("_custom"));
    }
