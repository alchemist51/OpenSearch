...
/**
 * Lucene-driver instruction handler. Reads a {@link ShardScanInstructionNode} produced
 * for a Lucene-driver StagePlan, acquires the shard's Lucene reader, deserializes the
 * filter {@link QueryBuilder} from {@code ShardScanExecutionContext.getFragmentBytes()},
 * compiles it to a Lucene {@link Query}, and returns a {@link LuceneCountExecutionContext}
 * for the {@code LuceneCountSearchExecEngine} to execute.
 *
...
final class LuceneScanInstructionHandler implements FragmentInstructionHandler<ShardScanInstructionNode> {
    ...
    public BackendExecutionContext apply(
        ...
        return new LuceneCountExecutionContext(searcher, decoded.filterQuery, decoded.columnNames);
    }
