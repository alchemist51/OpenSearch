...
/**
 * {@link EngineResultStream} for the Lucene-driver count fast path.
 *
 * <p>Structurally identical to
 * {@code org.opensearch.be.datafusion.DatafusionResultStream}: same
 * {@link BatchIterator} pump, same {@link ArrowResultBatch} wrapper, same
 * {@link Data#importIntoVectorSchemaRoot} call to materialise each batch into
 * a fresh {@link VectorSchemaRoot}. The only difference is the source of the
 * {@link ArrowArray}: DataFusion gets it from a native record-batch stream
 * (Rust → JNI), Lucene-driver exports a single-batch scratch VSR through the
 * C-Data interface so the resulting buffer layout matches DataFusion's
 * (foreign-allocation-managed buffers that survive Flight's
 * {@code VectorTransfer.transferRoot} hop).
 *
 * <p>Single-batch only: one count answer per shard. The iterator emits one
 * {@link EngineResultBatch} and is then exhausted.
 *
 * @opensearch.experimental
 */
@ExperimentalApi
public class LuceneCountResultStream implements EngineResultStream {

    /** C-Data array carrying the populated count batch. Owned by this stream until {@link #close}. */
     ...
     */
    public LuceneCountResultStream(ArrowArray arrowArray, ArrowSchema arrowSchema, BufferAllocator allocator) {
