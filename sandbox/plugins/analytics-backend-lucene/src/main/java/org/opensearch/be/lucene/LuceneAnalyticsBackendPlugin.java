...
public class LuceneAnalyticsBackendPlugin implements AnalyticsSearchBackendPlugin {
    ...
    public SearchExecEngineProvider getSearchExecEngineProvider() {
        return (ctx, backendContext) -> {
            if (!(backendContext instanceof LuceneCountExecutionContext luceneCtx)) {
                throw new IllegalStateException(
                    "Lucene SearchExecEngineProvider expected LuceneCountExecutionContext but got "
                        + (backendContext == null ? "null" : backendContext.getClass().getName())
                );
            }
            LuceneCountSearchExecEngine engine = new LuceneCountSearchExecEngine(luceneCtx);
            engine.prepare(ctx);
            return engine;
        };
    }
