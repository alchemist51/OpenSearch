...
/**
 * Pull-MV plugin: registers a target-owned shard build service for derived
 * indices with a {@link org.opensearch.cluster.metadata.DerivedIndexBinding}
 * and {@code mv_state} primary data format. The target uses the generic
 * data-format-aware primary/replica engines; only a started primary runs the
 * remote poller and DataFusion artifact build.
 *
 * <p>The deprecated {@code index.mv_pull.source_index} setting is registered
 * for BWC but is not read at runtime — source identity flows exclusively
 * through the public {@code index.derived.source.name} /
 * {@code index.derived.definition_id} settings enriched at creation time.
 */
