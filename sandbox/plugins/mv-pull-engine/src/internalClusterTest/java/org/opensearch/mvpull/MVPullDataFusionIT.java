...
import org.opensearch.mv.state.MVStateFormatShimPlugin;
...
public class MVPullDataFusionIT extends OpenSearchIntegTestCase {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins() {
        return List.of(
            ...
            MVStateFormatShimPlugin.class,
            LucenePlugin.class,
            DataFusionPlugin.class,
            Netty4ModulePlugin.class,
            MVPullEnginePlugin.class
        );
    }
    ...
    private void assertPrimaryOnlyBuildService() {
        ...
        String replicaNode = replicaNodeName();
        assertEquals("MV primary must run one poller", 1, buildService(primaryNode).activePollers());
        assertEquals("MV replica must not run a poller", 0, buildService(replicaNode).activePollers());
    }

    private MVShardBuildService buildService(String nodeName) {
        return internalCluster().getInstance(PluginsService.class, nodeName).filterPlugins(MVPullEnginePlugin.class).get(0).buildService();
    }
