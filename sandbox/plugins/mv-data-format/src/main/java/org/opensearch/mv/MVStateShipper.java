 ...
 *
 * @deprecated The separate-index ship path is superseded by the pull-based
 * flow. Retained for backward-compatible test coverage. TODO: remove once
 * pull-based flow is fully validated.
 */
@Deprecated
final class MVStateShipper implements DerivedStateReplicator {
    ...
    /**
     * Lock-free routing snapshot — used to resolve target shard ordinals
     * without calling {@code clusterService.state()} which is unsafe from
     * engine callbacks running on the cluster-applier thread.
     */
    private final TargetRoutingSnapshot routingSnapshot;
    ...
    MVStateShipper(
        ...
        String sourceIndex,
        ShardId shardId,
        TargetRoutingSnapshot routingSnapshot,
    ...
    public long replicate(VectorSchemaRoot stateBatch, BatchCoordinates coordinates) throws IOException {
        ...
        for (String targetIndex : targetIndices) {
            ...
            int targetShard = routingSnapshot.resolveTargetShard(targetIndex, shardId.id());
            if (targetShard < 0) {
                failure = new IOException(
                    "mv ship: target index [" + targetIndex + "] not found in routing snapshot version " + routingSnapshot.version()
                );
                shared.release();
                continue;
            }
