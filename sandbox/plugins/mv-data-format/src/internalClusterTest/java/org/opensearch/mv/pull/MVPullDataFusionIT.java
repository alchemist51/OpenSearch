...
public class MVPullDataFusionIT extends OpenSearchIntegTestCase {
            ...
            client().admin()
                ...
                .setSettings(
                    Settings.builder()
                        ...
                        // sort (read-side contract; admits sorted background merges).
     ...
     * the live generation count is eventually consolidated by a natural
     * merge, and the watermark never regresses.
     */
    @LockFeatureFlag(STREAM_TRANSPORT)
    public void testNaturalMergesUnderContinuousLoad() throws Exception {
        // Conditional admission: the target declares its group-by ordering as
        // a standard index sort, so DerivedIndexEngine admits background
        // merges and the generic sorted merge preserves the contract. The
        // global dataformat merge property remains the only scheduling gate.
        ...
        try {
            ...
            for (int wave = 0; wave < waves; wave++) {
                ...
                // Seal a source generation; the target's round pulls it and
                // publishes a small MV generation — and each target refresh
                // hands the scheduler a genuine merge opportunity.
                client().admin().indices().prepareRefresh(SOURCE).get();
            ...
            // Drain: full catch-up, exact answers on whatever mix of unmerged
            // and merged generations is live at this instant.
            final long drainedDocs = docs;
            assertBusy(() -> assertStateFilesEqual(expected, drainedDocs), 90, java.util.concurrent.TimeUnit.SECONDS);

            // Keep the load flowing until a NATURAL merge lands: every source
            // refresh publishes another generation (raising the max observed
            // live count), and a merge is the only mechanism that can move
            // the live count BELOW that maximum.
            final java.util.concurrent.atomic.AtomicLong docCounter = new java.util.concurrent.atomic.AtomicLong(docs);
            final java.util.concurrent.atomic.AtomicInteger maxObserved = new java.util.concurrent.atomic.AtomicInteger(0);
            assertBusy(() -> {
                for (int i = 0; i < docsPerWave; i++) {
                    long region = i % 4;
                    long adv = 1 + (i % 3);
                ...
                client().admin().indices().prepareRefresh(SOURCE).get();
                PublishedState observed = publishedStateOrNull(primaryNodeName(MV));
                assertNotNull("published state must be readable under load", observed);
                int live = observed.files().size();
                int max = maxObserved.accumulateAndGet(live, Math::max);
                assertTrue("waiting for a natural merge: live=" + live + " maxObserved=" + max, live < max);
            }, 120, java.util.concurrent.TimeUnit.SECONDS);

            // Post-merge: full catch-up again — answers must be exact on the
            // merged state, and the replica converges to the same catalog.
            final long totalDocs = docCounter.get();
    ...
    /**
     * Force-merge wave, then continued eligibility: a user force-merge is
     * ADMITTED on a sort-declared derived target (the same conditional gate
     * as background merges — see DataFormatAwareEngine#forceMerge) and
     * consolidates the accumulated generations through the same generic
     * sorted merge. Afterwards the target must remain fully live: new
     * publications keep landing, answers stay exact, and the merged fileset
     * wedges neither the poller nor subsequent merge admission.
     */
    @LockFeatureFlag(STREAM_TRANSPORT)
    public void testForceMergeWaveThenContinuedEligibility() throws Exception {
        ...
        try {
            ...
            // Phase 1: accumulate generations.
            Map<Long, long[]> expected = new HashMap<>();
            long docs = 0;
            final int docsPerWave = 8;
            ...
            final int filesBefore = publishedState(primaryNodeName(MV)).files().size();
            assertTrue("need multiple generations to consolidate: " + filesBefore, filesBefore > 1);

            // Phase 2: the force-merge wave. Admission is the engine's
            // conditional gate; the wave consolidates the fileset.
            var forceMerge = client().admin().indices().prepareForceMerge(MV).setMaxNumSegments(1).setFlush(true).get();
            assertEquals("admitted force-merge must not fail a shard", 0, forceMerge.getFailedShards());
            assertBusy(() -> {
                int live = publishedState(primaryNodeName(MV)).files().size();
                assertTrue("force-merge wave must consolidate: live=" + live + " before=" + filesBefore, live < filesBefore);
            }, 90, java.util.concurrent.TimeUnit.SECONDS);
            // Exactness on the force-merged state.
            assertBusy(() -> assertStateFilesEqual(expected, phase1Docs), 60, java.util.concurrent.TimeUnit.SECONDS);

            // Phase 3: continued eligibility — publication resumes on top of
            // the merged fileset and stays exact end-to-end.
