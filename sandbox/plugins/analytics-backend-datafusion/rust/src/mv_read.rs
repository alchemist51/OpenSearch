...
//! handler from the catalog snapshot's coverage), the prepare step calls
//! [`apply_mv_binding`] on the stripped Partial plan:
//!
//! ```text
...
//!
//! ## Source-side POC adaptation (leading-underscore state files)
//!
//! Hydrated state files on this branch are named
//! `_mv_partial.s<shard>.t<term>.g<gen>.<uuid>.parquet` (leading underscore).
//! The read path consumes the EXPLICIT absolute file list handed over from the
//! catalog snapshot — it never lists a directory — so DataFusion's hidden-file
//! rule (`_`/`.`-prefixed names) never applies (a non-collection
//! `ListingTableUrl` resolves via `ObjectStore::head`, not a directory list).
//! We also do NOT constrain `with_file_extension(".parquet")`: the extension
//! filter is set to empty so the explicit files are always admitted.
...
/// Shard-local MV coverage, set during MV-only session creation.
...
/// Builds a plan scanning the MV state files.
pub(crate) async fn build_mv_state_scan(
    ctx: &SessionContext,
    mv_file_paths: &[String],
) -> Result<Arc<dyn ExecutionPlan>> {
    build_mv_scan(ctx, mv_file_paths).await
}
...
async fn build_mv_scan(
    ...
    // underscore). Explicit file URLs bypass directory listing (and its
    // hidden-file rule), and an EMPTY extension filter admits them regardless
    // of suffix. `collect_stat(false)` because writer-alias column names would
    // otherwise resolve to all-null listing statistics and null-fill values.
