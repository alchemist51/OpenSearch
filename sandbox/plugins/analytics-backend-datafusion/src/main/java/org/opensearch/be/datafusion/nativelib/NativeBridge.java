    ...
    public static long createDatafusionReader(
        ...
        if (sortFields.size() != sortOrders.size()) {
            throw new IllegalArgumentException(
                "createDatafusionReader: sortFields (" + sortFields.size()
                    + ") and sortOrders (" + sortOrders.size() + ") must have the same length"
            );
        }
        try (var call = new NativeCall()) {
            var p = call.str(path);
            ...
            return call.invoke(
                CREATE_READER,
                p.segment(), p.len(),
                f.ptrs(), f.lens(), gens, f.count(),
                storePtr,
                sf.ptrs(), sf.lens(), so.ptrs(), so.lens(), sf.count()
            );
        }
    }
