 ...
 *
 * <ol>
 *   <li>{@link StageExecution.State#CREATED} — built by {@link LateMaterializationStageScheduler};
 ...
 * @opensearch.internal
 */
final class LateMaterializationStageExecution extends AbstractStageExecution implements DataConsumer, DataProducer {
    ...
    @Override
    public void start() {
        // TODO: implement Phases A, B, C, D — see class-level Javadoc.
        //   1. Phase A: drain childInputBuffer (block-read all K rows).
        //   2. Phase B: groupBy ___ugsi → fan out FetchByRowIdRequest per node via transport.
        //   3. Phase C: gather responses keyed by rowPos.
        //   4. Phase D: stitch + feed parentSink + close + transitionTo(SUCCEEDED).
        // Use ShardFragmentStageExecution's responseListener pattern for the fetch fan-out.
        // Use AbstractStageExecution.transitionTo(State) for state transitions.
        // Use captureFailure(Exception) + transitionTo(FAILED) on errors.
        throw new UnsupportedOperationException(
            "LateMaterializationStageExecution.start() is a skeleton — implement Phases A-D (see Javadoc)"
        );
    }
    ...
    public void cancel(String reason) {
        ...
        // Cancelling the parent task propagates to in-flight fetch transports + the
        // already-done child Sort+Limit stage. Same pattern as ShardFragmentStageExecution.
