...
final class DelegatedPredicateCombiner {
     ...
     *
     * <p>The {@link Delegated#subtree()} is either a single {@link AnnotatedPredicate} (leaf
     * case) or a raw {@code RexCall} that bubbled up an AND/OR/NOT of same-backend children
     * (the fused-bubble branches in {@link #classify}). For the performance-delegation
     * placeholder the driving backend evaluates the unwrapped expression natively, so every
     * {@code AnnotatedPredicate} marker on the way down has to come off — leaf, AND child,
     * OR child, anywhere in the tree.
     */
     ...
     * pass through. Used to make the performance-delegation placeholder Substrait-friendly —
     * the driving backend (e.g. DataFusion via Isthmus) doesn't know the {@code ANNOTATED_PREDICATE}
     * SqlOperator and chokes on it during plan conversion.
     */
