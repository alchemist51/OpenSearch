...
#[no_mangle]
pub extern "system" fn Java_org_opensearch_datafusion_DataFusionQueryJNI_startMemoryMonitor(
    mut env: JNIEnv,
    _class: JClass,
    output_file: JString,
    interval_ms: jlong,
    ...
    let file_path: String = env.get_string(&output_file).expect("Couldn't get file path").into();

    thread::spawn(move || {
        loop {
            if let Ok(allocated) = jemalloc_stats::refresh_allocated() {
                    ...
                    .open(&file_path)
                {
                    ...
                    let _ = writeln!(file, "Timestamp:: {}, Allocated:: {}", timestamp, allocated);
                }
            }
