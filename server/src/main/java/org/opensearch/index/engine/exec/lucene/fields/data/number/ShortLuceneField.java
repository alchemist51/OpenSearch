...
public class ShortLuceneField extends LuceneField {
    ...
    public void createField(FieldDescriptor descriptor, ParseContext.Document document, Object parseValue) {
        ...
        if (descriptor.isSearchable()) {
            document.add(new IntPoint(descriptor.fieldName(), value.shortValue()));
        }
        if (descriptor.hasDocValues()) {
            document.add(new SortedNumericDocValuesField(descriptor.fieldName(), value.shortValue()));
        }
        if (descriptor.isStored()) {
            document.add(new StoredField(descriptor.fieldName(), value.shortValue()));
        }
    }
