...
public class HalfFloatLuceneField extends LuceneField {
    ...
    public void createField(FieldDescriptor descriptor, ParseContext.Document document, Object parseValue) {
        ...
        if (descriptor.isSearchable()) {
            document.add(new HalfFloatPoint(descriptor.fieldName(), value.floatValue()));
        }
        if (descriptor.hasDocValues()) {
            document.add(new SortedNumericDocValuesField(descriptor.fieldName(), HalfFloatPoint.halfFloatToSortableShort(value.floatValue())));
        }
