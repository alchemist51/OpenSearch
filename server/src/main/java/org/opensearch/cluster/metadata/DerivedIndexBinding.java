...
public final class DerivedIndexBinding {
    ...
    /**
     * Canonical DERIVED DATA-FORMAT CATEGORY key. The single, immutable
     * declaration that a target index belongs to a derived data-format
     * category (e.g. {@code "materialized_view"}). This is NOT a physical
     * primary/secondary data format — it names a category whose target-artifact
     * format the composite store, pull service, and search path resolve through
     * the {@code DataFormatRegistry}. Registered as {@code IndexScope, Final}.
     *
     * <p>It is deliberately a first-class {@code index.derived.*} key rather
     * than an entry in {@code index.composite.secondary_data_formats}: a derived
     * category controls the control plane (registry lookup, eligibility,
     * creation, dispatch, validation), and must never be conflated with the
     * ordinary physical-format capability-assignment path.
     */
    public static final String KEY_DATA_FORMAT = "index.derived.data_format";

    /** Public Setting object for the derived data-format category (IndexScope, Final). */
    public static final Setting<String> SETTING_DATA_FORMAT = Setting.simpleString(
        KEY_DATA_FORMAT,
        Setting.Property.IndexScope,
        Setting.Property.Final
    );
    ...
    /**
     * Resolves the derived data-format category declared on an index, or
     * {@code null} if none is set. This is the canonical accessor every
     * control-plane consumer (composite store, pull eligibility, search
     * dispatch, validation) must use — never a string match against a
     * primary/secondary format list.
     *
     * @param settings the index settings
     * @return the category (e.g. {@code "materialized_view"}) or {@code null}
     */
    public static String dataFormatCategory(Settings settings) {
        String value = settings.get(KEY_DATA_FORMAT);
