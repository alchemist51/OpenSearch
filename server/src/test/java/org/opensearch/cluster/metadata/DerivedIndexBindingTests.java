...
public class DerivedIndexBindingTests extends OpenSearchTestCase {
    ...
    public void testPublicSettingsAreNotPrivate() {
        ...
        assertFalse("KEY_DATA_FORMAT must NOT be private", iss.isPrivateSetting(DerivedIndexBinding.KEY_DATA_FORMAT));
    }

    /**
     * The canonical derived data-format category setting must be a registered,
     * public IndexScope+Final setting so create-index requests may declare it.
     */
    public void testDataFormatCategorySettingRegisteredAndFinal() {
        IndexScopedSettings iss = new IndexScopedSettings(Settings.EMPTY, new HashSet<>(IndexScopedSettings.BUILT_IN_INDEX_SETTINGS));
        // isFinalSetting looks the setting up in the registry, so a true result
        // proves both registration AND that the category is immutable (Final).
        assertTrue(
            "index.derived.data_format must be a registered, Final index setting",
            iss.isFinalSetting(DerivedIndexBinding.KEY_DATA_FORMAT)
        );
    }

    /**
     * dataFormatCategory() resolves the declared category or null.
     */
    public void testDataFormatCategoryResolver() {
        assertNull("absent category resolves to null", DerivedIndexBinding.dataFormatCategory(Settings.EMPTY));
        assertNull(
            "empty category resolves to null",
            DerivedIndexBinding.dataFormatCategory(Settings.builder().put(DerivedIndexBinding.KEY_DATA_FORMAT, "").build())
        );
        assertEquals(
            "materialized_view",
            DerivedIndexBinding.dataFormatCategory(Settings.builder().put(DerivedIndexBinding.KEY_DATA_FORMAT, "materialized_view").build())
        );
    }
